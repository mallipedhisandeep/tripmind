...
export async function POST(req: NextRequest) {
  ...
  // ── Generate PDF via html-to-pdf ────────────────────────────
  // Using puppeteer-core + @sparticuz/chromium for Vercel serverless.
  ...
  try {
    ...
    // Fallback: return HTML if puppeteer not installed (local dev)
    return new NextResponse(html, {
      headers: {
        'Content-Type': 'text/html',
        'Content-Disposition': `attachment; filename="tripmind-${plan.destination}.html"`,
      },
    })
  }
  ...
  return new NextResponse(pdfBuffer, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="tripmind-${plan.destination}.pdf"`,
    },
  })
}
...
function buildPlanHtml(plan: GeneratedPlan, form: TripForm): string {
  const days = plan.days_plan?.map(day => `
    <div class="day">
      <h3>Day ${day.day} — ${day.date}: ${day.title}</h3>
      <table>
  ...
  const tips = plan.practical_tips?.map(t => `<li>${t}</li>`).join('') ?? ''
  const food = plan.food_recommendations?.map(f => `<li>${f}</li>`).join('') ?? ''
...
<body>
  <header>
    <h1>🗺️ ${plan.destination} Trip Plan</h1>
    <p>${form.from} → ${plan.destination} · ${plan.days} days · Generated by TripMind</p>
  </header>
  ...
  <div class="section">
    ...
    <div class="badges" style="margin-top:12px;">
      <span class="badge">👥 ${form.travelers} travellers</span>
      <span class="badge">💼 ${form.budget}</span>
      <span class="badge">🚂 ${form.transport}</span>
    </div>
    ${plan.crowd_warning ? `<div class="warn">⚠️ ${plan.crowd_warning}</div>` : ''}
    ${plan.weather_note  ? `<div class="warn">🌤️ ${plan.weather_note}</div>`  : ''}
  </div>

  <div class="section">
    <h2>Budget Estimate</h2>
    <div class="budget-grid">
      <div class="budget-box"><div class="label">Total (min)</div><div class="value">₹${plan.total_budget_min?.toLocaleString('en-IN')}</div></div>
      <div class="budget-box"><div class="label">Total (max)</div><div class="value">₹${plan.total_budget_max?.toLocaleString('en-IN')}</div></div>
      <div class="budget-box"><div class="label">Transport</div><div class="value">₹${plan.budget_breakdown?.transport?.toLocaleString('en-IN')}</div></div>
      <div class="budget-box"><div class="label">Stay</div><div class="value">₹${plan.budget_breakdown?.accommodation?.toLocaleString('en-IN')}</div></div>
      <div class="budget-box"><div class="label">Food</div><div class="value">₹${plan.budget_breakdown?.food?.toLocaleString('en-IN')}</div></div>
      <div class="budget-box"><div class="label">Activities</div><div class="value">₹${plan.budget_breakdown?.activities?.toLocaleString('en-IN')}</div></div>
    </div>
  </div>
